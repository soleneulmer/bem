...
def random_forest_regression(dataset,
                             model="output/r2_0.84_2019-07-23_17:05.pkl",
                             fit=False):
    ...
    if fit:
        ...
        name_Rf = 'output/r2_' + str(round(rf.best_score_, 2)) + '_' + str(datetime.datetime.now().strftime("%Y-%m-%d_%H:%M")) + '.pkl'
...
def computing_errorbars(regr, dataset_errors, train_test_sets):
    ...
    filename = 'output/test_radius_RF_errorbars.dat'
...
def predict_radius(regr,
                   my_planet=np.array([[1, 1, 0, 1, 5777, 1]]),
...
def plot_learning_curve(regr, dataset, save=False, fit=False):
    ...
    if fit:
        ...
        train_sizes = np.loadtxt(direc+'lc_train_sizes.dat')
        train_scores = np.loadtxt(direc+'lc_train_scores.dat')
        test_scores = np.loadtxt(direc+'lc_test_scores.dat')
    ...
    if save:
        np.savetxt(direc+'lc_train_sizes.dat', train_sizes)
        np.savetxt(direc+'lc_train_scores.dat', train_scores)
        np.savetxt(direc+'lc_test_scores.dat', test_scores)
    return plt
...
def plot_validation_curves(regr, dataset, name='features',
    ...
    if fit:
        ...
        if name == 'features':
            train_scores = np.loadtxt(direc+'vc_features_train_scores.dat')
            test_scores = np.loadtxt(direc+'vc_features_test_scores.dat')
        elif name == 'tree':
            train_scores = np.loadtxt(direc+'vc_tree_train_scores.dat')
            test_scores = np.loadtxt(direc+'vc_tree_test_scores.dat')
        elif name == 'depth':
            train_scores = np.loadtxt(direc+'vc_depth_train_scores.dat')
            test_scores = np.loadtxt(direc+'vc_depth_test_scores.dat')
        else:
            pass
    # Averaging
    train_scores_mean = np.mean(train_scores, axis=1)
    ...
    if save:
        if name == 'features':
            np.savetxt(direc+'vc_features_train_scores.dat', train_scores)
            np.savetxt(direc+'vc_features_test_scores.dat', test_scores)
        elif name == 'tree':
            np.savetxt(direc+'vc_tree_train_scores.dat', train_scores)
            np.savetxt(direc+'vc_tree_test_scores.dat', test_scores)
        elif name == 'depth':
            np.savetxt(direc+'vc_depth_train_scores.dat', train_scores)
            np.savetxt(direc+'vc_depth_test_scores.dat', test_scores)
        else:
            pass
