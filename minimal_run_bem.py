...
print_to_file(dataset, 'published_output', 'filtered_dataset.pkl')
...
regr, y_test_predict, _, train_test_sets = random_forest_regression(dataset)
...
plot_true_predicted(train_test_sets,
                        y_test_predict,
                        radii_test_output_error)

# Load the radial velocity dataset
dataset_rv = load_dataset_RV()
# Predict the radius of the RV dataset
radii_RV_RF = regr.predict(dataset_rv)
# Plot the predictions of the RV dataset
plot_dataset(dataset_rv, predicted_radii=radii_RV_RF, rv=True)


# Plot the learning curve
plot_learning_curve(regr, dataset)
# Plot the validation curves
plot_validation_curves(regr, dataset, name='features')
plot_validation_curves(regr, dataset, name='tree')
plot_validation_curves(regr, dataset, name='depth')

# Explain the RF predictions
exp = plot_LIME_predictions(regr, dataset, train_test_sets)

# Predict a new radius
# example given with GJ 357 b
radius, my_pred_planet = predict_radius(my_planet=np.array([[1.63, 0.1,
                                                                 0.034, 0.01,
                                                                 0.02, 0.005,
                                                                 0.337, 0.1,
                                                                 3505.0, 50,
